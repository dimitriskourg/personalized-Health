...
changeSettings.addEventListener('click', function(){
  $("body").load("./settings.html", function(){
    ...
    FinalUser.blooddonor === 1 ? document.querySelector("#yes").checked = true : document.querySelector("#no").checked = true;
  ...
  xhr.onload = function(){
    if(xhr.status === 200){
      ...
      for(let i = 0; i < allDoctors.length; i++){
        ...
        if(doctor.certified == 0)
          continue;//skip if not certified
...
AllInfo.addEventListener('show.bs.modal',function(event){
  ...
  if(info === "bmiInfo")
  {
    ...
    if(FinalUser.weight == 0 || FinalUser.height == 0){
      ...
      modalBody.innerHTML = "You need to set your height and weight to see your BMI";
      return;
    }
  ...
  <li class="list-group-item">Address: ${DOMPurify.sanitize(allDoctors[doctorInfo].address)}</li>
  <li class="list-group-item">Info: ${allDoctors[doctorInfo].doctor_info}</li>
  <li class="list-group-item">Speciality: ${allDoctors[doctorInfo].specialty}</li>
  <li class="list-group-item">Telephone: ${allDoctors[doctorInfo].telephone}</li>
</ul>`);
...
function BMIcalculator(cb,cb2){
  fetch(`https://fitness-calculator.p.rapidapi.com/bmi?age=${getAge(FinalUser.birthdate)}&weight=${FinalUser.weight}&height=${FinalUser.height}`, {
	"method": "GET",
	"headers": {
  ...
  fetch("https://fitness-calculator.p.rapidapi.com/idealweight?gender=male&height=180", {
	"method": "GET",
	"headers": {
		"x-rapidapi-host": "fitness-calculator.p.rapidapi.com",
		"x-rapidapi-key": "b3696e4eebmsh459a92cc3709e66p14cd8djsnd445f9a9aa9e"
