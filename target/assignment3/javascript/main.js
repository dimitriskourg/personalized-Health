...
function checkPasswordStrength(password){
    ...
    if(password.match(/[0-9]/g)!=null && password.match(/[0-9]/g).length / password.length > 0.5){
        return 0;
    }
    //removing duplicates using filter function
    //including only elements whose indexes match their indexOf value
    const uniqueChars = password.split('').filter((char, index) => password.indexOf(char) === index);
...
function finalCheck(){
    ...
    if(checkboxes.amka === false || checkboxes.username === false || checkboxes.email === false){
        // e.preventDefault()
        termsError.style.display = 'block';
